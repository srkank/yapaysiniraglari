...
function egtset_Callback(hObject, eventdata, handles)
    [file path]=uigetfile('*.mat','Eðitim dosyasýný seçiniz');
...
function calis_Callback(hObject, eventdata, handles)
    ...
    if(handles.tur == 2)
        ...
        isim = '                           Backpropagation YSA sonuçlarý';
    %else
    ...
    %    isim = '                           Perseptron YSA sonuçlarý';
    end
    sonuc = {isim; strcat('Dogru teþhis sayýsý :',num2str(dogru)); strcat('Yanlýþ teþhis sayýsý :',num2str(yanlis));strcat('Yanlýþ teþhis edilen hasta numaralarý :',num2str(yanlislar))};
